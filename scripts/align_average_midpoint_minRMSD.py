...
def align_da_to_z(pos, donor, acceptor):
    ...
    # angle of projection with x-axis; if the donor-acceptor axis already lies
    # along Z there is no XY projection to take an angle from, and the rotation
    # about Z is the identity
    xy_norm = math.sqrt(vec[0] ** 2 + vec[1] ** 2)
    if xy_norm < 1e-12:
        costhx, sinthx = 1.0, 0.0
    else:
        costhx = vec[0] / xy_norm
        sinthx = vec[1] / xy_norm
...
def main():
    ...
    # `rac`, the acceptor index in the *reactant* file. The correct index is
    # `pac`. It is left as-is so that averaged structures generated by earlier
    # versions of this script -- and the proton potentials derived from them --
    # stay reproducible. It only matters when rac != pac; fixing it would change
    # the averaged geometry and invalidate any downstream scan built on it.
