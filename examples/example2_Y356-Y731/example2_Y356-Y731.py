...
from autopcet import PCET, fit_bspline, kcal2eV, massH
...
for i, R in enumerate(Rs):
    rp_tmp, E_reac_tmp, E_prod_tmp = np.loadtxt(
        f"proton_potentials/R{R:.2f}_potential.dat", usecols=(0, 1, 2), unpack=True
    )
    rlim[i] = rp_tmp[-1]
...
for i, R in enumerate(Rs):
    ...
    for ii, (Ei, wfci) in enumerate(
        zip(
            Evib_reactant[:NStates_to_show], wfc_reactant[:NStates_to_show], strict=True
        )
    ):
        ...
        ax1.plot(
            rp, Ei + dEr + scale_wfc * sign * wfci, "b-", lw=1, alpha=(1 - 0.12 * ii)
        )
        ax1.fill_between(
    ...
    for ii, (Ei, wfci) in enumerate(
        ...
        sign = 1 if np.abs(np.max(wfci)) > np.abs(np.min(wfci)) else -1
        ax2.plot(
            rp, Ei + dEp + scale_wfc * sign * wfci, "r-", lw=1, alpha=(1 - 0.12 * ii)
        )
        ax2.fill_between(
    ...
    with open(f"rate_constant_contribution_R{R:.2f}A.log", "w") as outfp:
        ...
        outfp.write(f"\nR = {R:.2f}A\n")
        outfp.write(
            "(u, v)\t\tP_u\t\t\t|S_uv|\t\tDelta G_uv / eV\t\tDelta G^#_uv / eV\t% Contrib.\n"
        )
        outfp.write("-" * 125 + "\n")
        for u in range(NStates_to_show):
            for v in range(NStates_to_show):
                outfp.write(
                    f"({u:d}, {v:d})\t\t{Pu[u]:.3e}\t\t{np.abs(Suv[u, v]):.3e}\t\t{dGuv[u, v]:+.3f}\t\t\t{dGa_uv[u, v]:.3f}\t\t\t{percentage_contribution[u, v] * 100:.1f}\n"
                )
        outfp.write("=" * 125 + "\n\n")
...
plt.plot(
    R_fine_grid,
    kH_fine_grid / np.max(kH_fine_grid),
    "-",
    lw=2,
    color=color1,
    label=r"$k(R)$",
...
plt.plot(
    R_fine_grid,
    PR * kH_fine_grid / np.max(PR * kH_fine_grid),
    "-",
    lw=2,
    color="k",
    label=r"$P(R)k(R)$",
